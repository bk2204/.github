...
To find a binary in `PATH`, `command -v` should be used.
...
If the language does not have a standard formatting convention (e.g., C), four spaces should be used to indent.

== Documentation
:fn-direct-lang: pass:c,q[footnote:[As an example, https://www.canada.ca/en.html[the Canadian government's website] demonstrates well how to write in a simple direct way that is approachable to non-native speakers.]]
...
If a particular variant of AsciiDoc must be chosen, choose one that works with Asciidoctor.

We generally prefer U.S. English, and where applicable, a general Latin American Spanish and general European French.
Please consider that your audience may not be a native speaker of the language and write in a simple, direct way.{fn-direct-lang}
